...
#Start the server (compiled, [download] (https://github.com/sjqzhang/fastdfs/releases) speed experience, just one minute)
One-click installation: (Please copy the following command to the linux console to execute)
```shell
...
```
(Note: Pay attention to the version number of the link when downloading, run fileserver.exe directly under windows, and execute the file here [download] (https://github.com/sjqzhang/fastdfs/releases))
...
`http://yourserver ip:8080/upload.html` Note: Do not upload using 127.0.0.1
...
[More customers please refer to] (https://github.com/tus)



Deployment diagram
![Deployment diagram] (doc/go-fastdfs-deploy.png)

Universal file authentication timing diagram
![Common File Authentication Timing Chart] (doc/authentication2.png)

File google authentication timing diagram
![File Authentication Timing Chart] (doc/authentication.png)

# Please click [Feedback] (https://github.com/sjqzhang/go-fastdfs/issues/new)

# important
## There are a lot of actual questions and answers in the issue (many are closed, please check the closed issue)

## The project starts to stabilize from v1.1.8

# Release Notes
## From the low version to the high version, there may be configuration item changes, so be sure to pay attention to the configuration items when using the new version. How do I get the new version of the configuration items and instructions? Back up the old configuration item (the file name cannot be cfg.json), and then run the new version, the configuration item is automatically generated. Then modify the corresponding configuration item.

- v1.1.9 Added automatic file migration feature to support duplicate coverage options for files with the same name.
...
## Have questions, please add group
![QR code](doc/wechat.jpg)

#### Please enter the group to change the nickname, nickname format: city-company-nickname, if you like this project, please pay attention to (star) this project, attention is the affirmation of the project, but also the motivation of the author's innovation.
