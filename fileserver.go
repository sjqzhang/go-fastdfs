...
var FOLDERS = []string{DATA_DIR, STORE_DIR, STATIC_DIR}
...
var (
	...
	CONST_LEVELDB_FILE_NAME     = DATA_DIR + "/globalConfig.db"
...
<seelog type="asynctimer" asyncinterval="1000" minlevel="trace" maxlevel="error">  
	<outputs formatid="common">  
		<buffered formatid="common" size="1048576" flushperiod="1000">  
			<rollingfile type="size" filename="{DOCKER_DIR}log/globalConfig.log" maxsize="104857600" maxrolls="10"/>  
		</buffered>
	</outputs>  	  
	 <formats>
...
func Config() *globalConfig.GlobalConfig {
	return globalConfigHandler.Config()
}
...
func (this *Server) Reload(w http.ResponseWriter, r *http.Request) {
	...
	if action == "set" {
		...
		this.util.WriteFile(globalConfig.CONST_CONF_FILE_NAME, cfgjson)
		w.Write([]byte(this.util.JsonEncodePretty(result)))
		return
	}
	if action == "reload" {
		globalConfigHandler.ParseConfig()
...
func init() {
	...
	if e1 == nil && e2 == nil && appDir != curDir && !strings.Contains(os.Args[0], "go_build") {
		msg := fmt.Sprintf("please change directory to '%s' start globalConfig\n", appDir)
		msg = msg + fmt.Sprintf("请切换到 '%s' 目录启动 globalConfig ", appDir)
		log.Warn(msg)
		fmt.Println(msg)
		os.Exit(1)
	}
	...
	CONST_STAT_FILE_NAME = DATA_DIR + "/stat.json"
	CONST_SEARCH_FILE_NAME = DATA_DIR + "/search.txt"
	FOLDERS = []string{DATA_DIR, STORE_DIR, STATIC_DIR}
	...
	globalConfigHandler.InitGlobalConfig()
	...
	if ips, _ := server.util.GetAllIpsV4(); len(ips) > 0 {
		...
		if len(_ip) > 0 && !server.util.Contains(_ip[0], ips) {
			msg := fmt.Sprintf("host globalConfig is error,must in local ips:%s", strings.Join(ips, ","))
			log.Warn(msg)
			fmt.Println(msg)
		}
	}
...
func (this *Server) Start() {
	...
	if Config().EnableHttps {
		err := http.ListenAndServeTLS(Config().Addr, globalConfig.CONST_SERVER_CRT_FILE_NAME, globalConfig.CONST_SERVER_KEY_FILE_NAME, new(HttpHandler))
		log.Error(err)
		fmt.Println(err)
	} else {
		srv := &http.Server{
